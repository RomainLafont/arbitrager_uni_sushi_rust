...
pub struct SushiSwapPairWrapper {
    contract: SushiSwapPair<Provider<Http>>,
    decimals0: u8,
    decimals1: u8,
}

impl SushiSwapPairWrapper {
    pub fn new(address: Address, client: Arc<Provider<Http>>, decimals0: u8, decimals1: u8) -> Self {
        let contract = SushiSwapPair::new(address, client);
    ...
    pub async fn get_price(&self) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        println!("sushiswap: reserve0={} reserve1={}", reserve0, reserve1);
...
mod tests {
    ...
    async fn test_get_price() {
        ...
        let sushiswap_pair = SushiSwapPair::new("0x0000000000000000000000000000000000000000".parse().unwrap(), client, 18, 6);

        let price = sushiswap_pair.get_price().await.unwrap();
        assert_eq!(price, 0.5);
    }
}*/